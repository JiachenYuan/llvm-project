...
template <unsigned NumBits> struct LaneBitmaskImpl : public Bitset<NumBits> {
  ...
  constexpr LaneBitmaskImpl rotateRight(unsigned S) const {
    return rotateLeft(NumBits - (S % NumBits));
  }
...
private:
  ...
  friend struct format_provider<LaneBitmaskImpl>;
};
...
template <> struct format_provider<LaneBitmask> {
  static void format(const LaneBitmask &V, raw_ostream &Stream,
                     StringRef Style) {
    const auto &Data = V.getData();
    static constexpr unsigned SizeOfBitword = sizeof(Data[0]);
    static constexpr unsigned HexWidth = SizeOfBitword * 2;
    static constexpr unsigned NumWordsIn64Bits = 8 / SizeOfBitword;
    // Print only the first 64 bits if all upper words (anything beyond the
    // first 64-bits) are zero. Otherwise, print all words from most significant
    // to least significant.
    LaneBitmask UpperWords = ~LaneBitmask(~0ULL) & V;
    if (UpperWords.none())
